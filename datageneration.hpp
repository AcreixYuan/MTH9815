//
// Created by Suyang Yuan on 12/24/23.
//

#ifndef FINAL_PROJECT_DATAGENERATION_HPP
#define FINAL_PROJECT_DATAGENERATION_HPP
#include "soa.hpp"
...
void GenerateTradeData()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        for (int j = 0; j < 10; j++)
        {
            ...
            string side = "SELL";
            string price = ConvertPrice(100.0);
            if (tradecount % 2)
            {
                side = "BUY";
                price = ConvertPrice(99.0);
            }
...
#endif //FINAL_PROJECT_DATAGENERATION_HPP
